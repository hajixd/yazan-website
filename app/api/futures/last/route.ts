...
const RECENT_BAR_WINDOW_MS = 6 * 60 * 60_000;
...
const buildDatabentoBarsUrl = (databentoSymbol: string, startMs: number, endMs: number) => {
  const url = new URL(DATABENTO_HISTORICAL_URL);
...
const parseLatestBarClose = (payload: string) => {
  const lines = payload.split("\n");
  let latestBar: { price: number; time: number } | null = null;
  ...
  for (const rawLine of lines) {
    ...
    try {
      const record = JSON.parse(line) as DatabentoBarRecord;
      const bucketStartTime = record.hd?.ts_event ? Date.parse(record.hd.ts_event) : NaN;
      const price = normalizeNumber(record.close);

      if (!Number.isFinite(bucketStartTime) || !Number.isFinite(price)) {
        continue;
      }

      const time = bucketStartTime + 60_000 - 1;

      if (!latestBar || time >= latestBar.time) {
        latestBar = { price, time };
      }
    } catch {
      continue;
    }
...
export async function GET(request: Request) {
  ...
  try {
    ...
    try {
      ...
      let activeEndMs = requestedEndMs;
      let activeStartMs = Math.max(0, activeEndMs - RECENT_BAR_WINDOW_MS);
      let { response, payload } = await fetchDatabentoText(
        buildDatabentoBarsUrl(asset.databentoSymbol, activeStartMs, activeEndMs),
        authHeaders
      );
      ...
      if (!response.ok) {
        ...
        if (availableEndMs !== null && activeEndMs > availableEndMs) {
          activeEndMs = availableEndMs;
          activeStartMs = Math.max(0, activeEndMs - RECENT_BAR_WINDOW_MS);
          ({ response, payload } = await fetchDatabentoText(
            buildDatabentoBarsUrl(asset.databentoSymbol, activeStartMs, activeEndMs),
            authHeaders
          ));
      ...
      if (response.ok) {
        const latestBar = parseLatestBarClose(payload);
        ...
        if (latestBar) {
          ...
          storeLatestTradeCache(asset, responsePayload);
          console.warn(
            `[databento:last] Serving latest 1m bar close for ${asset.symbol} after trade lookup failure: ${errorMessage}`
          );
          return NextResponse.json(responsePayload, { headers });
        }
      }
